...
// Test the secondary temperature gauge with a range of values
function testSecondaryTemperatureGauge() {
    console.log('Testing secondary temperature gauge...');
    
    // Test with values across the full range
    const tempValues = [5, 10, 15, 20, 30, 40];
    let index = 0;
    
    // Update the gauge with each value at intervals
    const intervalId = setInterval(() => {
        const temp = tempValues[index];
        console.log(`Setting secondary temperature to ${temp}°C`);
...
// Test the humidity gauge with a range of values
function testHumidityGauge() {
    console.log('Testing humidity gauge...');
    
    // Test with values across the full range
    const humidityValues = [0, 20, 40, 60, 80, 100];
    let index = 0;
...
// Test the pressure gauge with a range of values
function testPressureGauge() {
    console.log('Testing pressure gauge...');
    
    // Test with values across the full range
    const pressureValues = [950, 980, 1000, 1013, 1030, 1050];
    let index = 0;
    
    // Update the gauge with each value at intervals
    const intervalId = setInterval(() => {
        const pressure = pressureValues[index];
        console.log(`Setting pressure to ${pressure} hPa`);
        updatePressureGauge(pressure);
        
        index++;
        if (index >= pressureValues.length) {
            clearInterval(intervalId);
